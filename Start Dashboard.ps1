...
if ($health -and $health.app -ne 'ak-branch-monitor') { throw 'Port 4387 belongs to another application.' }
if (-not $health) {
    ...
    for ($attempt=0; $attempt -lt 20; $attempt++) {
        ...
        try { $health = Invoke-RestMethod "$dashboardUrl/health" -TimeoutSec 2; if ($health.app -eq 'ak-branch-monitor') { break } } catch {}
    }
    if ($health.app -ne 'ak-branch-monitor') { throw 'Dashboard did not become ready. Check server-error.log.' }
}
