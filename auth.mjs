...
export function createAuth({username,password,secret,passwordHash}={}){
  ...
  const configured=Boolean(signingKey);
  ...
  const issue=(identity=expectedUser,now=Date.now(),ttlMs=8*60*60*1000)=>{
    if(!configured)throw Error('Application login is not configured.');
    if(typeof identity==='number'){ttlMs=now;now=identity;identity=expectedUser;}
    const usernameValue=typeof identity==='string'?identity:identity?.username;
    if(!usernameValue)throw Error('Session identity is required.');
    const payload=Buffer.from(JSON.stringify({u:usernameValue,e:now+ttlMs})).toString('base64url');
  ...
  const validate=(token,now=Date.now())=>{
    ...
    try{
      ...
      return typeof decoded.u==='string'&&Number(decoded.e)>now?decoded:null;
    }catch{return null;}
  };
