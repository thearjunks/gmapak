...
const root=path.dirname(fileURLToPath(import.meta.url)),project=path.join(root,'dashboard'),dist=path.join(root,'public'),privateDir=path.resolve(process.env.AK_DATA_DIR||path.join(root,'.private'));
...
const users=createUserStore({file:path.join(privateDir,'users.json'),bootstrapUsername:process.env.APP_USERNAME,bootstrapPassword:process.env.APP_PASSWORD,bootstrapPasswordHash:process.env.APP_PASSWORD_SHA256});
...
const session=req=>{const signed=auth.validate(cookies(req).ak_session);if(!signed)return null;const user=users.getByUsername(signed.u);return user?.active?user:null;};
const has=(user,permission)=>Boolean(user?.permissions?.includes(permission)||user?.roles?.includes('super_admin'));
const requirePermission=(res,user,permission)=>{if(has(user,permission))return true;json(res,403,{message:'You do not have permission to access this feature.'});return false;};
const snapshotFor=screen=>{const snapshot=read(snapshotPath,{}),queries=snapshot.queries??{},reviewRows=queries.reviews?.rows??[],accessSummary={reviewBodies:reviewRows.length,branchesWithReviewBodies:new Set(reviewRows.map(review=>review.branchId)).size};if(screen==='overview'||screen==='branches')return {...snapshot,accessSummary,queries:{branches:queries.branches,reviews:{...(queries.reviews??{}),rows:[]}}};if(screen==='connection'||screen==='users')return {...snapshot,accessSummary,queries:{branches:{...(queries.branches??{}),rows:[]},reviews:{...(queries.reviews??{}),rows:[]}}};return {...snapshot,accessSummary};};
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(req.method==='GET'&&url.pathname==='/health'){const snapshot=read(snapshotPath,{});return json(res,200,{status:'ok',app:'ak-branch-monitor',authConfigured:auth.configured&&users.configured,branches:snapshot.queries?.branches?.rows?.length??0,refreshSource:snapshot.sync?.source||'Google Maps public listings'});}
 if(req.method==='POST'&&url.pathname==='/api/auth/login'){
  if(!auth.configured||!users.configured)return json(res,503,{message:'Application login is not configured.'});
  const credentials=await body(req),user=users.authenticate(credentials.username,credentials.password);if(!user)return json(res,401,{message:'Incorrect username or password.'});
  return json(res,200,{authenticated:true,username:user.username},{'Set-Cookie':sessionCookie(auth.issue(user.username))});
 }
 ...
 if(req.method==='GET'&&url.pathname==='/api/auth/session')return json(res,200,{authenticated:true,...signedIn});
 ...
 if(req.method==='GET'&&url.pathname==='/api/snapshot'){const screen=url.searchParams.get('screen')||'overview';if(!requirePermission(res,signedIn,`screen:${screen}`))return;return json(res,200,snapshotFor(screen));}
 if(req.method==='GET'&&url.pathname==='/api/google/status')return json(res,200,publicStatus());
 if(req.method==='POST'&&url.pathname==='/api/google/refresh'){if(!requirePermission(res,signedIn,'data.refresh'))return;if(state.syncing)return json(res,202,{message:'A refresh is already running.'});sync().catch(()=>{});return json(res,202,{message:'Google refresh started. Official Business Profile data is preferred when API access is available.'});}
 if(req.method==='POST'&&url.pathname==='/api/google/connect'){
  if(!requirePermission(res,signedIn,'google.connect'))return;
 ...
 if(req.method==='GET'&&url.pathname==='/api/admin/users'){if(!requirePermission(res,signedIn,'users.view'))return;return json(res,200,{users:users.list()});}
 if(req.method==='POST'&&url.pathname==='/api/admin/users'){if(!requirePermission(res,signedIn,'users.manage'))return;const created=users.create(signedIn,await body(req));return json(res,201,{user:created});}
 const userMatch=url.pathname.match(/^\/api\/admin\/users\/([0-9a-f-]+)$/i);
 if(req.method==='PATCH'&&userMatch){if(!requirePermission(res,signedIn,'users.manage'))return;const updated=users.update(signedIn,userMatch[1],await body(req));return json(res,200,{user:updated});}
 ...
 }catch(e){const message=e.message||'Request failed.',code=/already exists/i.test(message)?409:/User not found/i.test(message)?404:/Password|Username|Super Admin|permission/i.test(message)?400:500;json(res,code,{message});}});
