...
const config=()=>{const c=read(path.join(privateDir,'google-client.json'),{});return c.web??c;};
...
const connected=()=>configured()&&Boolean(read(tokenPath,{}).refresh_token);
const saveState=()=>atomic(path.join(privateDir,'sync-status.json'),state);
const publicStatus=()=>({...state,configured:configured(),connected:connected(),source:'Google Maps public listings',groupId:GROUP,redirectUri,refreshMinutes:15,snapshotVersion:read(snapshotPath,{}).generatedAt,message:state.message});
...
async function sync(){if(state.syncing)return;state.syncing=true;state.lastAttempt=new Date().toISOString();state.message='Reading public Google Maps ratings and review counts…';saveState();let old;
 try{old=read(snapshotPath,null);const next=await refreshPublicSnapshot(old,new GoogleMapsPublicClient());atomic(snapshotPath,next);state.lastSuccess=next.generatedAt;state.message=`Updated ${next.sync.succeeded} branches from public Google Maps; ${next.sync.failed} branches failed or were ambiguous and retain previous data.`;
...
const server=http.createServer(async(req,res)=>{try{
 ...
 if(req.method==='GET'&&url.pathname==='/health')return json(res,200,{status:'ok',app:'stc-branch-dashboard',branches:read(snapshotPath,{}).queries?.branches?.rows?.length??0,refreshSource:'Google Maps public listings'});
 ...
 if(req.method==='POST'&&url.pathname==='/api/google/refresh'){if(state.syncing)return json(res,202,{message:'A refresh is already running.'});sync().catch(()=>{});return json(res,202,{message:'Google Maps refresh started. Ratings and review counts will appear automatically when ready.'});}
